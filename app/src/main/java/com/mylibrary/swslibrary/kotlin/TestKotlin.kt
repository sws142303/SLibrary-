...
fun wtsk(){
     ...
     * 2.通过（o instanceof Map）判断如果不是map势力 返回false
     * 3.判断两个map的长度是否相同 如果不相等 返回false
     * 4.接下来会遍历MapA 判断MapA中的keyMapB是否包含 不包含的话返回false 如果包含的话会将key在MapA和MapB中对应的value取出来 判断value是否相同 如果不相同则返回false
     ...
     */
    val listOf1 = listOf(0,1,2,3,4,5)
    val listOf2 = listOf(0,2,1,5,4,3)
    SLog.print("listOf1 == listOf2 : ${listOf1 == listOf2}")
    SLog.print("listOf1 == listOf2 : ${listOf1.equals(listOf2)}")
